...
async function openUrlInSimulatorSafeAsync({
  ...
  try {
    if (devClient) {
      bundleIdentifier = await (0, _BundleIdentifier().configureBundleIdentifierAsync)(projectRoot, exp);
      await assertDevClientInstalledAsync(simulator, bundleIdentifier); // stream logs before opening the client.
...
async function streamLogsAsync({
  ...
  const imageName = await SimControlLogs().getImageNameFromBundleIdentifierAsync(udid, bundleIdentifier);

  if (imageName) {
    // Attach simulator log observer
    SimControlLogs().streamLogs({
      pid: imageName,
      udid
    });
  }
