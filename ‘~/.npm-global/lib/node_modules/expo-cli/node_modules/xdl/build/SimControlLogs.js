...
function parseMessageJson(data) {
  ...
  try {
    ...
    _xdl().Logger.global.debug('Failed to parse simctl JSON message:\n' + stringData);
  }
  ...
  childProcess.stdout.on('data', data => {
    ...
    if (simLog.messageType === 'Error') {
      ...
      !isReactLog(simLog) && !isCoreTelephonyLog(simLog) && !isWebKitLog(simLog) && !isRunningBoardServicesLog(simLog)) {
        ...
        _xdl().Logger.global.error(formatMessage(simLog));
      }
    } else if (simLog.eventMessage) {
      var _simLog$source4;

      // If the source has a file (i.e. not a system log).
      if (((_simLog$source4 = simLog.source) === null || _simLog$source4 === void 0 ? void 0 : _simLog$source4.file) || simLog.eventMessage.includes('Terminating app due to uncaught exception')) {
        hasLogged = true;

        _xdl().Logger.global.info(formatMessage(simLog));
      }
    }

    if (!hasLogged) {
      _xdl().Logger.global.debug(formatMessage(simLog));
    } else {// console.log('DATA:', simLog);
    }
  });
  childProcess.on('error', ({
    message
  }) => {
    _xdl().Logger.global.debug('[simctl error]:', message);
  });
...
//# sourceMappingURL=__sourcemaps__/SimControlLogs.js.map
